...
**Three bullets in the sweep stay yours.** Recording changes made outside git, adding newly discovered follow-ups, and writing down work agreed in discussion but not started all take this session as their input. The agent cannot see any of it, so it is scoped out rather than left to report a confident nothing. They land in the step 9 commit along with the agents' own findings.

## Step 10 — what re-runs

`yarn validate` always. An agent only where the step 9 commit touched a path that was not in the diff it read — a fix confined to files it has already seen does not earn a second pass, and one that pulls in new surface does. That is a condition you can check rather than a judgement you have to make.
