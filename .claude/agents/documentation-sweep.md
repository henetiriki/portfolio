...
- Any new doc is listed in [`docs/README.md`](../../docs/README.md), which nothing checks.
...
- [`docs/decisions/`](../../docs/decisions/README.md) carries a file only where the change makes a choice that would otherwise be re-litigated — one that cannot name the alternative it discarded has not earned its place. Any file it adds does not contradict an existing one; where it revises one, its `Status:` names what it supersedes and the superseded file's `Status:` names it back. A merged decision whose body has been edited is a defect, not a tidy-up.
- **Explanatory comments are documentation too, wherever they sit** — `src/`, `e2e/`, `scripts/`, the service worker, the config files — **and are checked in both directions.** A comment describing code that has since changed is the same defect as a stale doc, and nothing mechanical catches it. Read every comment the change touched against the code beside it, and any comment the change made stale even where the comment itself was not edited. **Apply the same test to a comment that you would to a doc**: one that still makes sense with the code deleted is a document in the wrong place. See D-260905c.
