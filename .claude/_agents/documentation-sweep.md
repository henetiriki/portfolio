...
- **Apply the same test to a comment that you would to a doc**: one that still makes sense with the code deleted is a document in the wrong place. See D-260905c.
- **A comment running past four lines is a finding on length alone**, whatever else it passes. Say where it should go instead — a caveat about what a check cannot see belongs in the topical doc beside that check, and what is left at the call site is one line plus the pointer.
