...
- **Comment the code where it reads as wrong or arbitrary, and nowhere else.** Either it looks like a mistake and is not — `body { background-color: transparent }` is load-bearing — or it looks arbitrary and is not. Say what you discarded and why it fails; the code already says what it does. **A comment that still makes sense with the code deleted is in the wrong place**, because that is a document and a comment should be unintelligible alone. Where the argument genuinely needs a page, it goes in a doc and the comment is one line plus a pointer. This replaces a ban on comments in CSS, CSS Modules and JSX — see D-260905c.
- **Topical docs are reference; the log is argument.** A doc holds only what neither the code nor its comment can — cross-file facts, provenance, constraints spanning several files. Bullets by default, one fact each, stated first. **If a sentence restates something that exists elsewhere, delete it rather than compressing it**: a summary is a copy, and the copy is what drifts, because nobody updates a summary. "In brief", "in short" and "essentially" announce the duplication.
