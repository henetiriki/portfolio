...
- **Topical docs are reference; [`docs/decisions.md`](docs/decisions.md) is argument.** A doc holds only what neither the code nor its comment can — cross-file facts, provenance, constraints that span several files. Bullets by default, one fact each, stated first. **If a sentence restates something that exists elsewhere, delete it rather than compressing it**: a summary is a copy, and the copy is what drifts, because nobody updates a summary. "In brief", "in short" and "essentially" announce the duplication.
