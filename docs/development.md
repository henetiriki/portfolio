...
Jest + React Testing Library, set up 2026-08-06 (see [Roadmap](roadmap.md) — the harness and the full tiered coverage pass across the codebase are both done, ~84% whole-project coverage).
...
- **`src/utils/test/googleMapsMock.ts`** is a hand-built mock of the `google.maps` SDK surface, covering only what `Map`/`Marker`/`Polyline`/`MapWrapper` actually call (not the full real API) — `Map`, `Marker`, `Polyline`, `InfoWindow`, `LatLng`, `LatLngBounds`, `Point`, `Animation`, `event.addListener`/`removeListener`, `geometry.encoding.decodePath`. Each mock class's methods are `jest.fn()`s (so assertions can use `toHaveBeenCalledWith`) and tracks its own instances in a `static instances[]` array (so a test can grab the specific instance a component created, e.g. `const [marker] = MockMarker.instances`) — call `installGoogleMapsMock()` once per test file and `resetGoogleMapsMock()` in `beforeEach` to clear the instance arrays between tests (`clearMocks: true` already resets the `jest.fn()` call histories, but not the arrays themselves). `triggerMapsEvent(target, eventName, ...args)` fires a synthetic event (`'zoom_changed'`, `'click'`, `'visible'`) on a specific mock instance — the real `google.maps.event` system is a global registry keyed by `(target, eventName)`, not per-instance emitters, so the mock replicates that with a `WeakMap`.
  - **Gotcha**: `jest.useFakeTimers()` combined with a mock whose method synchronously re-triggers the event that a `cancelableDelay`-based recursive call is waiting on (`Map.tsx`'s `zoomMap` — each step registers a `zoom_changed` listener, awaits `delay(80)`, then calls `setZoom`, which the mock uses to fire `zoom_changed` synchronously, re-entering `zoomMap`) can spiral into a runaway recursion under `jest.advanceTimersByTimeAsync()` and OOM the whole test process. Don't try to drive the full multi-step recursive animation to completion in a test — asserting the _trigger_ fired correctly (e.g. `map.panTo` was called) is enough; see `Map.test.tsx`'s "pans to Auckland" test.
