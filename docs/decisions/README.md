...
- **Never cite a checklist step by its number.** A body cannot be corrected when the sequence renumbers, so the citation is wrong for good — name the step instead. Two live entries already carry a stale "step 9" from the renumber of 2026-09-07, which is the evidence rather than a hypothetical.
