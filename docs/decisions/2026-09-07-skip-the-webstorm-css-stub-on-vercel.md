...
**Not a blanket early return either.** The same script serves both modes, so returning before the argv check would make `css-vars:check` exit `0` without checking anything — a check that cannot fail, which is worse than no check. `VERCEL` is unset in GitHub Actions, so CI would not have caught the difference.

**`css-vars:check` earns its CI step, and this change is what makes the reason clear.** The roadmap item raising this doubted it, on the grounds that with no committed copy the check can only prove generation from `colors.ts` is deterministic. That undersells it: what it actually proves is that the install produced current output at all. That was already the invariant it caught in the worktree case above, and it is now the invariant this change makes conditional — a guard widened past generate mode, or an early return that stopped the write, would leave CI with no stub and a failing check. Cheap, gated behind the documentation-only path, and the only automated thing watching the mechanism above.
