...
| `WaveWrapper`   | Renders one of the four normally loaded decorative wave SVGs (`content-top`, `content-bottom`, `footer-top`, `footer-bottom`) from `public/images/waves` as a full-width background-cover image inside a flex box (10rem tall by default, overridable via `style`). |
