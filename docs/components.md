...
| `Transition`      | Full-screen dark overlay with a Mantine `Loader` (`type='points'`), shown while `useLoading()` reports a Next.js route change in progress.                                                                                                                                                                                                                                  |

## `nav/`

| Component        | Responsibility                                                                                                                                                                                                                                                              |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Navigation`     | Sticky header with `Logo`, desktop inline menu (hidden below `sm`), a `Burger`-triggered mobile `Drawer`, and a floating scroll-to-top button that appears once `window.scrollY > 10`. Background becomes opaque (`blackRussian`) once scrolled or when the drawer is open. |
