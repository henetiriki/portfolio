...
- Added weekly Dependabot version updates for the Yarn 4 dependency graph and GitHub Actions. Minor and patch packages are grouped by production/development scope, Action updates are grouped separately, majors remain individually reviewable, and known TypeScript/Babel/Node-type routine major updates are explicitly ignored until their roadmap prerequisites are met without suppressing security updates.
