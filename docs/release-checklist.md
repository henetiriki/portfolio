...
6. **Dispatch the [code review](#code-review)**, and carry on — it reads in a context of its own and does not block step 7. Steps 9 onwards do wait for it.
...
**The review's findings are committed before the agents are dispatched, which is the point of splitting steps 8 and 9.** The agents then read a complete, review-corrected diff, rather than one that changes underneath them while they work — a stale read costs a re-dispatch, and a re-dispatch is minutes. It also keeps the two signals apart: a review reports on what is inside the diff, the agents on how the diff sits against the rest of the tree, and blended into one commit that distinction is lost. The cost is that steps 9 to 11 sit behind the review, which is the price of the stable diff and worth saying rather than implying the review blocks nothing. It is a bounded wait rather than a stall, because the review is dispatched to a context of its own and reads while `yarn validate` runs; where a person has to run it instead, that same wait becomes a round trip of unbounded length.
...
**Dispatched before `yarn validate` rather than after it**, so it reads the diff while the production build and the browser suite run, and so its findings arrive before a build has been paid for. `yarn validate` does not wait for it and should not be made to, but the two finishing independently is not the same as this section being done: the pull request waits for the review to report even when the checks are already green.

> Claude Code dispatches a subagent with an empty context to invoke its bundled `code-review` skill, rather than invoking it from [`release-ready-check`](../.claude/skills/release-ready-check/SKILL.md) directly — see [the dispatch decision](decisions/2026-09-06-cap-automatic-subagent-dispatch.md). A person runs it instead only where no subagent can be dispatched. **Unlike the two sections below, there is deliberately no brief here to fall back on.** A review's criteria belong to the reviewing tool, so writing one out would invent a method this repository does not have and would drift from what the skill actually does. A tool without that skill has no equivalent step here and should say so rather than improvise one — the only step on this page that does not survive being read by another tool.
