...
>
> **It judges the tip of what was pushed, not the change as a whole.** `HEAD^ HEAD` is one commit wide, and Vercel creates a deployment per push rather than per commit — so a push whose last commit is documentation skips the build for everything else in that push. A branch that deploys fine while commits arrive one at a time stops deploying when the same commits arrive together, which a rebase or any batched push does routinely. Observed on [#196](https://github.com/henetiriki/portfolio/pull/196), where a force-push after a rebase left a documentation commit on top and the pull request got no preview at all. Whether to change this is [open work](roadmap.md).
>
