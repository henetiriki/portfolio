...
- [ ] **Introduce a Content Security Policy.** Ship it in Report-Only first and promote it only once the reports are clean, because Maps, BotID, Vercel telemetry and remote images all need an allowlist. The deprecated `X-XSS-Protection` header has already been removed, so this is the remaining half of the security-header work.
  - **A Maps request already fails on `/travel`, and it is not ours.** PageSpeed on 2026-08-14 logged `net::ERR_BLOCKED_BY_CLIENT` for `https://maps.googleapis.com/maps/api/mapsjs/gen_204?csp_test=true`; in an ordinary browser the same request surfaces as `ERR_FAILED`. That one error is the entire reason the page scores **Best Practices 96** — `errors-in-console` scores 0 and nothing else on the page fails. No CSP is deployed today, so **shipping one will not fix this**, and the failure must not later be misread as CSP fallout. It is a `gen_204` beacon, which is the shape client-side blockers filter by default.
  - **Which is why the console cannot be the Report-Only signal.** The SDK sends that probe to discover whether a policy blocks its own requests, so under Report-Only it looks like the obvious thing to watch — but it already fails without a policy, and a blocked probe cannot distinguish our header from the client's blocker. Judge the rollout on the violation reports themselves.
  - **Allowlist `maps.googleapis.com` explicitly**, rather than discovering it through a Report-Only violation that a blocker may have caused instead.
