...
- [ ] **Decide the dash style for ranges.** The 2026-08-29 copy pass normalised sentence dashes in user-facing copy to the em-dash character `—`, matching the SEO descriptions, portfolio alt text and the logo. Ranges were left as spaced hyphens: `TimelineFromTo`'s `{from} - {to}`, the footer copyright `2014 - {year}`, and the rail-trip route labels in `src/fixtures/travel/railTrips.ts`. The typographically correct choice for a range is the en-dash `–`, not `—`; changing `TimelineFromTo` and the footer would also touch `Footer.test.tsx` and the `experience-work-history` visual baseline.
