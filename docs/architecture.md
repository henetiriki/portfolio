...
2. `src/pages/_app.tsx` (`Portfolio`) wraps every page:
   ...
   - Renders, in order: an optional route-transition overlay (`Transition`, driven by `useLoading`), the `DynamicFixedBackground` (full-bleed Instagram-photo background), `Navigation`, then `Layout` wrapping the actual page `<Component>`.
   - `FixedBackground` is loaded via `next/dynamic` with `ssr: false`: it calls `useLayoutEffect`, which errors during SSR, and its background image id is only known after a client-only fetch, so there is nothing meaningful to render server-side anyway. `Transition` is a plain static import — it's trivial first-party markup with no external dependencies, so code-splitting it would only delay the overlay appearing behind an extra chunk fetch on the very first route change, working against its purpose as instant navigation feedback. Ordinary content remains server-renderable.
...
- `/api/rail-trips` serves static rail trip data (`@fixtures/travel/railTrips`) as edge-cacheable JSON, kept as an API route rather than a direct import so it isn't bundled into client JS eagerly; see [Travel Feature](travel-feature.md#rail-trips-api-round-trip) for how `useRailTrips` consumes it.
