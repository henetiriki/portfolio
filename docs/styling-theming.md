...
- Wave dividers (`WaveWrapper`) are pre-rendered SVG files under `public/images/waves/`, not generated CSS — swapping the visual "torn paper" section transition would mean editing those SVGs rather than the component. `WaveWrapper` takes a plain `style` prop (not `sx` — that prop doesn't exist in v7) for the couple of call sites that need to override its height/margin. See "`WaveWrapper`'s height override" below for a real bug this shape of API ran into.
...
- **Never let an autoformatter/refactor tool rewrite `$mantine-breakpoint-*` to `var(--mantine-breakpoint-*)` inside an `@media` condition.** They look interchangeable but aren't: `$mantine-breakpoint-*` (`Navigation.module.css`, `NavigationLink.module.css`, `portfolio.module.css`) is a `postcss-simple-vars` **build-time text substitution** — it compiles to a literal value (`48em`, `67.5em`, ...) before the browser ever sees it. `var(--mantine-breakpoint-*)` is a real CSS custom property, and `@media (min-width: var(--x))` is syntactically accepted but **never matches in any browser** — `var()` resolution doesn't apply inside a media feature. This happened twice during the v7 migration (an automated rewrite silently broke every responsive breakpoint in the app both times) and was only caught by checking `matchMedia()` in a real browser, not by lint/type-check/tests. If a breakpoint needs to come from a CSS custom property instead (e.g. exposed at `:root` for JS to read too), it has to go through a `postcss-simple-vars` variable for the `@media` condition specifically — there's no way around the substitution requirement there.
...
- **`html:root[data-mantine-color-scheme='dark'] { --mantine-color-error: ... }`** — v7's default dark-scheme `--mantine-color-error` is `red-8` (`#e03131`); this app's v6 setup used the lighter `red-4` (`#ff8787`) for validation-error text and required-field asterisks. Overriding the variable is more resilient than chasing every component that consumes it, but the selector matters: Mantine's own rule is `:root[data-mantine-color-scheme="dark"]`, and **`:root` is a pseudo-class** (class-level specificity, `(0,1,0)`), not an element selector — so a plain `html[data-mantine-color-scheme='dark']` override (specificity `(0,1,1)`, one element + one attribute) actually still _loses_ to Mantine's `(0,2,0)` (one pseudo-class + one attribute). This cost a real debugging round-trip: the override compiled fine and the CSS variable existed, but silently never took effect. Matching `:root`'s specificity by including it alongside `html` (`html:root[...]`, `(0,2,1)`) wins regardless of stylesheet load order.

### Component-level fixes (not in `global.css`)

- **`WaveWrapper`'s height override.** `WaveWrapper` used to set a hardcoded `h='10rem'` Box style-prop _and_ merge a caller-supplied `style` object (for the one call site — the footer's top wave — that needs a shorter `5rem`). Mantine v7's `getBoxStyle` (`@mantine/core/core/Box/get-box-style`) always spreads style-props **after** the `style` object: `{ ...style, ...vars, ...styleProps }`. That means a shorthand prop like `h` unconditionally wins over anything the same component's own `style` prop sets for that CSS property — the footer's top wave rendered at 160px instead of the intended 80px, visibly taller than production and throwing off the logo's position relative to it. Fixed by moving the default into the `style` object itself (`style={{ height: '10rem', ...style }}`) instead of a separate `h` prop, so a caller's `style={{ height: '5rem' }}` override actually applies.
