...
- **Static root styles with a matching Mantine style prop**: prefer the prop, e.g. `c='silver.4'`, `h='75vh'`, `display='none'`, or `pos='fixed'`. Besides being shorter, color-aware props resolve theme names without spelling out a CSS variable. Use `style` for unsupported properties such as `borderRadius`, `overflow`, `visibility`, `zIndex`, or a dynamic one-sided border; do not contort a supported shorthand into a subtly different declaration just to eliminate `style`.
...
- **`WaveWrapper`'s height override.** `WaveWrapper` used to set a hardcoded `h='10rem'` Box style-prop _and_ merge a caller-supplied `style` value (for the one call site — the footer's top wave — that needs a shorter `5rem`). Mantine v7's `getBoxStyle` (`@mantine/core/core/Box/get-box-style`) always spreads style-props **after** the `style` value: `{ ...style, ...vars, ...styleProps }`. That means a shorthand prop like `h` unconditionally wins over anything the same component's own `style` prop sets for that CSS property — the footer's top wave rendered at 160px instead of the intended 80px, visibly taller than production and throwing off the logo's position relative to it. The default now lives in the first entry of Mantine's documented style array (`style={[{ height: '10rem', overflow: 'hidden' }, style]}`), so an object, callback, or array supplied by the caller is merged later and can override it. A focused component test locks down the `5rem` override.
