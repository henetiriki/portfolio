...
`Navigation` (`@components/nav/Navigation.tsx`) renders `menuItems` from `@fixtures/nav` (`Home`, `Experience`, `Portfolio`, `Travel`, `Contact`) as a sticky header, collapsing to a full-screen `Drawer` on mobile — see [Components](components.md#nav) for the breakpoint, link sizing and scroll-to-top behaviour, and [State Management](state-management.md#actions) for the `pageTopRef` scroll target. `Footer` renders the same `menuItems` as a secondary nav.
