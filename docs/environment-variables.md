...
| Variable                                                  | Consumed by                                                                      | Purpose                                                                                                                                                                                                                                                   |
| --------------------------------------------------------- | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CSP_VIOLATION_EMAILS`                                    | `server/csp-mail/mail.ts`                                                        | `true` — and only exactly that — emails surviving Content Security Policy violations for a monitoring window; off everywhere else, read at module load so a change needs a redeploy. Temporary; see [Security](security.md#emailing-violations-temporary) |
...
| `GMAIL_SENDER_EMAIL`                                      | `server/contact/helpers.ts`, `server/contact/send.ts`, `server/csp-mail/mail.ts` | The address contact-form messages are sent _to_ (the owner's inbox) and the transporter's `from` address; violation emails go to the same address with a `+csp` tag                                                                                       |
