...
## D-260822d — Update visual baselines from a default-branch workflow, not a pull request
...
**The workflow definition runs from `main`, while the tested checkout comes from the named target branch.** GitHub only accepts `workflow_dispatch` from a workflow present on the default branch, which makes `main` the reviewed implementation boundary. Letting an operator select a pull-request ref for the workflow itself would give that ref an opportunity to rewrite the token-bearing commands before the run. The generator instead validates the branch input, then checks that branch out only for the build and test.

**Only the commit job receives `contents: write`, and it never executes target-branch code.** The repository's Actions default remains read-only; the generator has no write token and passes an allowlisted artifact to the commit job. That job rechecks the target revision and every path before copying PNGs into a clean checkout and pushing the commit. It does not create or approve pull requests, accepts no arbitrary shell input, and fails if a test generated a change outside the baseline PNG allowlist. This keeps CI's Linux render the source of truth without introducing a personal access token or widening the normal CI token.
...
**`scripts/check-doc-links.mjs` follows the existing `css-vars:check`/`icons:check` shape** — no dependencies, walks the docs, fails non-zero on a mismatch — but sits on the opposite side of the docs-only gate. Those two skip on a documentation-only change because they check generated _source_ artifacts that prose can't affect; this one exists specifically to check docs, so it runs unconditionally alongside `prettier:check`, the other ungated step, per the reasoning already established in [D-260815c](#d-260815c--give-documentation-only-changes-a-cheap-ci-path-rather-than-no-ci-run).
