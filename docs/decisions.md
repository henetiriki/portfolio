...
**`docs/` and `chore/` are verifiable claims; `feature/` and `fix/` are not.** This repository already classifies changes twice — CI's cheap path and Vercel's `ignoreCommand` — and `docs/` matches the first exactly while both skip the second. That makes the prefix a prediction the build either confirms or contradicts, which is a property a naming convention does not usually get. `feature/` and `fix/` were kept regardless: the set has to cover every branch or people fall back to bare names, and once a prefix is mandatory the common cases need somewhere to go.
